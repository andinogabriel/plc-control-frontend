            ...
            <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
              ...
              <Tooltip title="Descargar PNG">
                <span><IconButton size="small" disabled={points.length === 0}
                  onClick={() => exportChartPng(tempChartRef.current, 'temperatura.png', { title: 'Temperatura vs tiempo', source: 'Mediciones', legend: [{ label: 'Temperatura (°C)', color: theme.palette.primary.main }, ...(config ? [{ label: 'Banda objetivo', color: theme.palette.success.main, area: true }] : [])] })}
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <Card><CardContent ref={humChartRef}>
            <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
              ...
              <Tooltip title="Descargar PNG">
                <span><IconButton size="small" disabled={points.length === 0}
                  onClick={() => exportChartPng(humChartRef.current, 'humedad.png', { title: 'Humedad vs tiempo', source: 'Mediciones', legend: [{ label: 'Humedad (%)', color: theme.palette.secondary.main }, ...(config ? [{ label: 'Banda objetivo', color: theme.palette.success.main, area: true }] : [])] })}
                  aria-label="Descargar gráfico">
                  <ImageRoundedIcon fontSize="small" /></IconButton></span>
              </Tooltip>
            </Stack>
