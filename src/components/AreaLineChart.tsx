...
export function AreaLineChart({
  ...
  // Cluster vertical markers that fall within ~2% of the visible span: a burst of config changes
  // otherwise draws an unreadable picket fence of dashed lines. Each cluster renders as one line
  // labelled xN; markers outside the visible (zoomed) window are dropped. Zooming in re-runs this,
  // so a cluster naturally splits back into individual markers as the span narrows.
  const markers = (() => {
    const sorted = [...(verticalMarkers ?? [])].sort((a, b) => a.date.getTime() - b.date.getTime());
    if (sorted.length < 2 || viewLabels.length < 2) return sorted;
    const t0 = viewLabels[0].getTime();
    const t1 = viewLabels[viewLabels.length - 1].getTime();
    const span = t1 - t0;
    const visible = span > 0 ? sorted.filter((m) => m.date.getTime() >= t0 && m.date.getTime() <= t1) : sorted;
    if (span <= 0) return visible;
    const minGap = span * 0.02;
    const out: VerticalMarker[] = [];
    let cluster: VerticalMarker[] = [];
    const flush = () => {
      if (cluster.length === 0) return;
      out.push(cluster.length === 1 ? cluster[0] : { ...cluster[0], label: `x${cluster.length}` });
      cluster = [];
    };
    for (const m of visible) {
      if (cluster.length > 0 && m.date.getTime() - cluster[cluster.length - 1].date.getTime() > minGap) flush();
      cluster.push(m);
    }
    flush();
    return out;
