...
/**
 * Serialises the first SVG inside {@code container} to a PNG and downloads it. The chart's
 * computed colours are read so the export matches the current theme.
 */
export async function exportChartPng(container: HTMLElement | null, filename: string) {
  ...
  // Paint a solid background matching the card so transparent PNGs are readable anywhere.
